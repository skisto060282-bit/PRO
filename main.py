...
"""
    # ---- أزرار فرعية (Inline) تظهر مع الرسالة ----
    ...
    # أرسل الترحيب مع الأزرار الفرعية Inline
    bot.send_message(chat_id, welcome_msg, reply_markup=inline_kb, parse_mode="Markdown")

    # ---- أزرار رئيسية ثابتة أسفل الشاشة (ReplyKeyboard) ----
    main_keyboard = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    btn_start = telebot.types.KeyboardButton('/start')
    btn_stop = telebot.types.KeyboardButton('/stop')
    main_keyboard.add(btn_start, btn_stop)

    # نرسل رسالة قصيرة لتثبيت شريط الأزرار السفلي
    bot.send_message(chat_id, "🔘 شريط الأزرار الرئيسية أسفل الشاشة.", reply_markup=main_keyboard)
...
def handle_message(message):
    ...
    # إذا المستخدم ضغط /start أو /stop عبر شريط الأزرار الأسفل فهي أوامر موجودة وتعالج سابقاً
    # هنا نتعامل مع الروابط، والـ IPs، وغيرها.
    ...
    # معالجة IPs (يمكن إرسال IP واحد أو قائمة)
    ip_list = []
    ...
    if ip_list:
        ...
        # في حال لم يكن نص رسالة IP أو رابط، نتجاهل أو نعلّم المستخدم
        # (لا نغيّر سلوك القديم — هذه رسالة إرشادية بسيطة)
        bot.send_message(chat_id, "❌ لم يتم التعرف على أي IP صالح أو أمر مفهوم. اضغط /start لعرض الخيارات.")
