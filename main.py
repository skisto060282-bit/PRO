...
def check_http(ip, port):
    try:
        response = requests.get(f'http://{ip}:{port}', timeout=HTTP_TIMEOUT,
...
def check_https(ip, port):
    try:
        response = requests.get(f'https://{ip}:{port}', timeout=HTTP_TIMEOUT,
...
def smart_proxy_scan(ip, port):
    ...
    if check_http(ip, port): protocols.append("HTTP")
    if check_https(ip, port): protocols.append("HTTPS")
    if check_connect(ip, port): protocols.append("CONNECT")
...
"""
            ...
            if ip_data and criticalASN in ip_data.get('as',''):
                bot.send_message(chat_id, f"🚨🚨 تنبيه عاجل! وجد بروكسي ضمن ASN المهم جداً {criticalASN} — IP: {ip}")
            return True
    return False
...
def process_bulk_quick_scan(chat_id, ip_list):
    ...
    scanned_count = 0
    active_count = 0
    progress_msg = bot.send_message(chat_id, f"🔄 **جاري الفحص النشط**\n✅ 0/{total_ips}\n🟢 0 ✅\n📊 0% ▱▱▱▱▱▱▱▱▱▱")
    with ThreadPoolExecutor(max_workers=SCAN_CONCURRENCY) as executor:
        futures = []
        for item in ip_list:
            if user_operations[chat_id]['stop']: break
...
"""
                    try: bot.edit_message_text(progress_text, chat_id, progress_msg.message_id)
                    except: pass
            except: scanned_count+=1
...
"""
    ...
    if chat_id in user_operations: del user_operations[chat_id]
...
def get_ssh_account_sync():
    try:
        ...
        else: return f"❌ خطأ {r.status_code}"
...
🎮 **الأزرار الفرعية:**"""
    ...
    reply_kb=telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
    reply_kb.row('/start','/stop')
    
    bot.send_message(chat_id,welcome_msg,reply_markup=keyboard)
    bot.send_message(chat_id,"🎯 أزرار ثابتة:",reply_markup=reply_kb)
...
def stop_message(message):
    ...
    else: bot.send_message(chat_id,"⚠️ لا توجد عمليات فحص جارية")
...
def handle_message(message):
    ...
    for line in text.split('\n'):
        ...
        if ':' in line and validate_ip(line.split(':')[0]):
            ...
            try: ports=[int(p) for p in port_str.split(',')] if ',' in port_str else [int(port_str)]
            except: ports=defaultPorts
    ...
    if ip_list:
        ...
        else: bot.send_message(chat_id,"🔍 **جاري الفحص السريع...**")
        threading.Thread(target=process_bulk_quick_scan,args=(chat_id,ip_list)).start()
    else: bot.send_message(chat_id,"❌ لم يتم التعرف على أي IP صالح")
